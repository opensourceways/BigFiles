...
func CheckRepoOwner(userInRepo UserInRepo) (Repo, error) {
	...
	headers := http.Header{contentType: []string{headerContentType}}
	repo := new(Repo)
	...
	headersGitCode := http.Header{contentType: []string{headerContentType}}
...
func GetAccountManageToken() (string, error) {
	...
	if managerTokenOutput.STATUS != 200 {
		msg := fmt.Sprintf(": get manager token response fail, "+
